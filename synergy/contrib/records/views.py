...
class CreateRecordView(RegionViewMixin, RecordViewMixin, CUMessagesMixin, CreateView):
    ...
    def get_form_class(self, *args, **kwargs):
        setup = self.get_record_setup(**self.kwargs)
        return forms.createform_factory(setup, setup.model.model_class(), setup.related_o2m_models.all(), setup.related_m2m_models.all(), setup.use_model_m2m_fields, excluded_fields=self.get_excluded_fields(), hidden_fields=self.get_hidden_fields())
...
class UpdateRecordView(RecordViewMixin, RegionViewMixin, CUMessagesMixin, UpdateView):
    ...
    def get_initial(self):
        return self.get_record_setup(**self.kwargs).get_update_initial(**self.get_arguments())
