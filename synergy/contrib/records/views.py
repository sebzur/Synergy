...
class RecordViewMixin(RecordComponentViewMixin):
    ...
    def get_record_setup(self, **kwargs):
        return get_model('records', 'RecordSetup').objects.using(get_frontend_db(self)).get(name=kwargs.get('name'))
    ...
    def get_hidden_fields(self):
        setup = self.get_record_setup(**self.kwargs)
        return setup.routed_fields.filter(is_hidden=True).values_list('field', flat=True)
